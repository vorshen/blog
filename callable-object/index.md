...
Handle<Code> code =
      JSEntry(isolate, params.execution_target, params.is_construct);
  {
    ...
    // ...
```
Code 对象非常的重要，这个就是 v8 中函数执行的关键，v8 相关原话是:  
...
```
[generated bytecode for function:  (0x06c008212561 <SharedFunctionInfo>)] // 1⃣️
Parameter count 1
Register count 3
Frame size 24
         ...
         0x6c008212637 @   17 : 5d f9 02          CallUndefinedReceiver0 r1, [2] // 3⃣️
...
[generated bytecode for function: drink (0x06c0082125b9 <SharedFunctionInfo drink>)] // 2⃣️
Parameter count 1
Register count 3
Frame size 24
...
2⃣️那里有一个 drink 关键字，代表是我们显示声明的函数；1⃣️那里就是整段 js 代码，被作为了一个匿名函数执行。  
_3⃣️就是调用 drink 的地方。_

不过 js 本身是一个函数式编程语言，函数式是如何表现的我们不谈，重点说一说「闭包」，闭包一词不可能有前端开发不知道(_哪怕没用过，面试也遇到过_)，那我们思考一下，为什么闭包可以跨越栈帧遍历的限制？  
以下面这个函数为例:  
```
...
drink();
```
如果使用 d8 输出字节码，可以看到总共有三个 generated bytecode for function，整段执行的过程，猜测的话函数执行作用域变化应该如下:    
![](./assets/1.png)  
这里总共有三个阶段，重点看后面两个，第二阶段是执行了匿名的自执行函数，此时声明了一个 flag 变量在对应的作用域。第三阶段是执行 drink 函数，这里用到了两个变量。  
