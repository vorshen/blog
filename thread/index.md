...
让人又爱又恨的 chrome，每个 tab 都默认情况下是单独的进程，流畅是真的，内存大户也是真的。

### 线程
结合进程的特点，可以显而易见发现进程的劣势在于资源的同步，因为互相隔离，无法共享内存，那样一些数据共享的场景，就需要借助 xxx 手段来完成，增加了成本。
...
一个优秀的程序员可以不脱发，但是不能不追求性能，所以协程应运而生。协程的特点就是它是应用层面的，不再交由操作系统去切换处理，由开发着自己控制，可以尽情的想办法优化。
...
# 多线程编程
### hello world
上面从刚刚介绍了进程、线程、协程的区别，下面就开始真正的线程介绍。

Talk is cheap. Show me the code，我们用斐波拉切数列为例，使用 C++ 和 Javascript 展示一下多线程计算的代码。
...
# 多线程的坑
如上可知多线程好处多多，那是不是应该无脑整上，千万要谨慎，因为多线程坑还是很多的，下面列举几个
